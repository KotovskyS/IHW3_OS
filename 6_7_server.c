...
int main(int argc, char *argv[]) {
  if (argc != 6) {
    printf("Usage: %s <Server IP> <Server Port> <Text> <Client Count> <Observer Port>\n", argv[0]);
    exit(1);
  }

  struct sockaddr_in server_addr, observer_addr;
  int server_socket, observer_socket, observer;
  int num_clients = atoi(argv[4]);  // Number of clients defined by command line argument
  ...
  for (int i = 0; i < num_clients; i++) {
    ...
    // Send text and fragment info to client
